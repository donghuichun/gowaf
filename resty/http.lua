...
local function _format_request(self, params)
    ...
    -- Initialize request
    local req = {str_upper(params.method), " ", self.path_prefix or "", params.path, query, HTTP[version],
...
local function _receive_status(sock)
    ...
    if not version then
        return nil, nil, nil, "couldn't parse HTTP version from response status line: " .. line
    end

    local status = tonumber(str_sub(line, 10, 12))
    if not status then
        return nil, nil, nil, "couldn't parse status code from response status line: " .. line
    end
